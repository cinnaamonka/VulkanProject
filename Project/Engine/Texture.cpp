...
void Texture::CreateTextureImage(const VkDevice& device, const VkPhysicalDevice& physicalDevice,
...
void Texture::CreateImage(const VkDevice& device, uint32_t width, uint32_t height, const VkFormat& format,
...
VkCommandBuffer& Texture::BeginSingleTimeCommands(const VkDevice& device, const VkCommandPool& commandPool)
{
	...
	vkAllocateCommandBuffers(device, &allocInfo, &m_TempCommandBuffer);

	VkCommandBufferBeginInfo beginInfo{};
	beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
	beginInfo.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;

	vkBeginCommandBuffer(m_TempCommandBuffer, &beginInfo);

	return m_TempCommandBuffer;
}

void Texture::EndSingleTimeCommands(const VkDevice& device, const VkCommandBuffer& commandBuffer,
...
void Texture::TransitionImageLayout(const VkDevice& device, const VkCommandPool& commandPool,
...
void Texture::CopyBufferToImage(const VkBuffer& buffer, const VkImage& image, uint32_t width,
...
VkImageView& Texture::CreateImageView(const VkImage& image, const VkFormat& format, const VkDevice& device,
...
void Texture::CreateTextureSampler(const VkDevice& device, const VkPhysicalDevice& physicalDevice)
{
